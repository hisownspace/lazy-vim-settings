return {
  ...
  opts = {
    keymaps = {
      toggle = "<leader>cx", -- Keybind to toggle Codex window (Disabled by default, watch out for conflicts)
      quit = "<C-q>", -- Keybind to close the Codex window (default: Ctrl + q)
    }, -- Disable internal default keymap (<leader>cc -> :CodexToggle)
    border = "rounded", -- Options: 'single', 'double', or 'rounded'
    width = 0.8, -- Width of the floating window (0.0 to 1.0)
    height = 0.8, -- ight of the floating window (0.0 to 1.0)
    model = "gpt-5-mini", -- Optional: pass a string to use a specific model (e.g., 'o3-mini')
